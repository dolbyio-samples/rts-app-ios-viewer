...
final actor VideoTracksManager {
    ...
    func observeLayerUpdates(for source: StreamSource) {
        Task { [weak self] in
            guard let self, await self.layerEventsObservationDictionary[source.sourceId] == nil else {
                return
            }
    ...
    func queueEnableTrack(for source: StreamSource, layer: MCRTSRemoteVideoTrackLayer? = nil) async throws {
        ...
        guard let serialTasks = sourceToTasks[source.sourceId], source.videoTrack.isActive else { return }
